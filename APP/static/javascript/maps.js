...
document.getElementById("addCampusBtn").addEventListener("click", () => {
  ...
  map.once("click", (e) => {
    ...
    campusMarker = L.marker([lat, lng], { icon: schoolIcon, draggable: true })
      .addTo(map)
      .bindPopup(`
    ...
    setTimeout(() => {
      // submit
      document.getElementById("campusForm").addEventListener("submit", async (event) => {
      ...
      // cancel
      document.getElementById("cancelCampusBtn").addEventListener("click", () => {
        map.removeLayer(campusMarker)
        campusMarker = null
        isAddingCampus = false
      })
...
async function initializeMap() {
  ...
  const geocoder = L.Control.geocoder({
    ...
    geocoder: L.Control.Geocoder.nominatim({
      ...
      viewbox: [
        [122.5, 9.0],
        [126.0, 11.5],
      ], // approximate bounding box for Cebu region
      bounded: 1,
    }),
  }).addTo(map)
  ...
  geocoder.on("markgeocode", (e) => {
    ...
    const poly = L.polygon([bbox.getSouthEast(), bbox.getNorthEast(), bbox.getNorthWest(), bbox.getSouthWest()]).addTo(
      map,
    )
  ...
  // Modify your map click handler to automatically find nearest campus
  map.on("click", (e) => {
    // Clear previous routes when starting a new route
    clearRoute()
    ...
    const nearestCampus = findNearestCampus(endPoint)
    if (nearestCampus) {
...
function calculateRoute(startPoint, endPoint) {
    ...
    loadingDiv.style.cssText =
      "position: absolute; top: 10px; left: 50%; transform: translateX(-50%); background: white; padding: 10px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.2); z-index: 1000;"
...
function calculateDetour(startPoint, endPoint) {
    ...
    const numPoints = 16 // Number of points to try
    const waypoints = []
    ...
    distanceFactors.forEach((factor) => {
      for (let i = 0; i < numPoints; i++) {
        ...
        const waypointLng =
          center.lng + ((radius * factor) / (111000 * Math.cos(center.lat * (Math.PI / 180)))) * Math.sin(angle)
...
function tryNextWaypoint(startPoint, endPoint, waypoints, index) {
    // Get the loading indicator if it exists
    const loadingDiv = document.getElementById("route-loading")

    // Update loading message if the element exists
    if (loadingDiv) {
      loadingDiv.innerHTML = `<div class="spinner"></div><p>Calculating detour route... (${index + 1}/${waypoints.length})</p>`
    }
    ...
    if (index >= waypoints.length) {
      ...
      fetch(originalUrl)
        ...
        .catch((error) => {
          console.error("Error fetching original route data:", error)
          // Remove loading indicator if it exists
          if (loadingDiv) {
            document.body.removeChild(loadingDiv)
          }
    ...
    if (routeData && routeData.routes && routeData.routes.length > 0) {
      ...
      if (route.geometry) {
        ...
        const routeStyle = isWarningRoute
          ? { color: "#ff3333", weight: 6, opacity: 0.8, dashArray: "10, 10" }
          : { color: "#3388ff", weight: 6, opacity: 0.8 }

        // Add the route to the routesLayer instead of directly to the map
        const routeLayer = L.geoJSON(route.geometry, {
          style: routeStyle,
        })

        // Add the route to routesLayer
        ...
        if (coordinates.length > 0) {
          ...
          const popup = L.popup({ closeButton: true, autoClose: false }).setLatLng(midpoint).setContent(popupContent)

          routesLayer.addLayer(popup)
          popup.openOn(map)
        }
...
function initializePopulationDistribution() {
  ...
  radiusSlider.addEventListener("input", () => {
    const newRadius = Number.parseInt(radiusSlider.value, 10)
      ...
      style: {
        color:
          report.type === "flood"
            ...
            : report.type === "strike"
              ? "red"
              : report.type === "mobility restriction"
                ? "green"
                : report.type === "fire"
                  ? "orange"
                  : "gray",
...
function updateAffectedStudents() {
  ...
  allMarkers.forEach((marker) => {
    ...
    for (const areaType in affectedAreas) {
      if (affectedAreas[areaType]) {
        // check if areaType array exists
...
function isStudentAffected(student) {
  return (
    affectedStudents.flood.includes(student) ||
...
function addSchoolMarkers() {
  ...
  schools.forEach((school) => {
    const marker = L.marker([school.lat, school.lng], { icon: schoolIcon }).bindPopup(`<b>${school.name}</b>`)
...
async function plotPreviousSchools() {
  ...
  Object.values(groupedSchools).forEach((school) => {
    ...
    if (latitude && longitude && !isNaN(Number.parseFloat(latitude)) && !isNaN(Number.parseFloat(longitude))) {
      ...
      const marker = L.marker([Number.parseFloat(latitude), Number.parseFloat(longitude)], {
        icon: schoolIcon,
...
function setupRoutingControls() {
  // Track the current routing mode
  let currentRoutingMode = "student-campus"

  // Add event listeners to the routing mode dropdown options
  document.querySelectorAll(".routing-mode-option").forEach((option) => {
    option.addEventListener("click", function (e) {
      e.preventDefault()

      // Remove active class from all options
      document.querySelectorAll(".routing-mode-option").forEach((opt) => {
      ...
      // Update the current routing mode
      currentRoutingMode = this.dataset.mode

      // Clear any existing routes
      clearRoute()

      console.log(`Routing mode changed to: ${currentRoutingMode}`)
    })
  })

  // Modify the map click handler based on routing mode
  map.on("click", (e) => {
    if (currentRoutingMode === "student-campus") {
      // Existing behavior for student to campus routing
      // This is already handled in your code
    } else if (currentRoutingMode === "campus-affected") {
      // New behavior for campus to affected area routing
      if (incidentCircle) {
        // Clear previous routes
        clearRoute()

        // Set end point to incident circle center
        endPoint = incidentCircle.getLatLng()
        ...
        if (nearestCampus) {
          ...
          // Calculate route between nearest campus and incident location
          calculateRoute(startPoint, endPoint)
        } else {
          alert("No campus data available. Please ensure campuses are loaded.")
        }
      } else {
        alert("Please add an incident location first using the Incident Location (Circle) tool.")
      }
    }
  })
}
...
function updateYearFilter() {
  ...
  checkboxes.forEach((checkbox) => {
    if (checkbox.checked) {
      currentFilters.yearLevel = Number.parseInt(checkbox.dataset.year)
    }
  })
}

function updateAgeFilter(event) {
  currentFilters.age = event.target.value ? Number.parseInt(event.target.value) : null
}
...
function clearFilters() {
  ...
  document
    .querySelectorAll("#previous-school-checks .form-check-input")
...
function applyFilters() {
  ...
  if (
    currentFilters.strand.size === 0 &&
    currentFilters.course.size === 0 &&
    currentFilters.previous_school.size === 0 &&
    !currentFilters.yearLevel &&
    !currentFilters.age
  ) {
...
function showMarker(student) {
  ...
  if (currentFilters.previous_school.size > 0 && !currentFilters.previous_school.has(student.previous_school))
    return false
...
function generatePopupContent(student, isAffected) {
  ...
  if (isAffected) {
    ...
    if (affectedStudents.restricted.includes(student))
      content += '<span class="restricted-tag">Mobility Restriction</span> '
...
function createPopulationDistribution(data) {
  ...
  const radiusSlider = document.getElementById("radius-slider")
  const initialRadius = Number.parseInt(radiusSlider.value, 10)
  ...
  radiusSlider.addEventListener("input", function () {
    const radius = Number.parseInt(this.value, 10)
  ...
  if (data && data.length > 0) {
    studentCount = data.filter((student) => {
      ...
      if (
        !student.latitude ||
        !student.longitude ||
        isNaN(Number.parseFloat(student.latitude)) ||
        isNaN(Number.parseFloat(student.longitude))
      ) {
        return false
      }
      ...
      const distance =
        calculateDistance(
          school.lat,
          school.lng,
          Number.parseFloat(student.latitude),
          Number.parseFloat(student.longitude),
...
function studentType(student) {
  ...
  if (student.department) {
    return Object.values(COLLEGE_DEPARTMENTS).some((depts) => depts.includes(student.department))
  }
...
function handleStudentRoute(student, focusRoute = true) {
  const studentLat = Number.parseFloat(student.latitude)
  const studentLng = Number.parseFloat(student.longitude)
  ...
  const waypoints = [L.latLng(studentLat, studentLng), L.latLng(campusLat, campusLng)]

  let routeColor = "blue"
  if (typeof student === "object") {
    routeColor = studentType(student) ? "blue" : "green"
  } else if (endPoint && endPoint === incidentCircle?.getLatLng()) {
    // If routing to an incident area, use red
    routeColor = "#ff3333"
  }
  ...
  const clickedMarker = markers.getLayers().find((marker) => marker.options.studentData === student)
  ...
  if (clickedMarker && clickedMarker.isPopupOpen()) {
    clickedMarker.setPopupContent(generatePopupContent(student, isStudentAffected(student)))
  }
}
    ...
    if (currentRouteMarker.isPopupOpen()) {
      currentRouteMarker.setPopupContent(
        generatePopupContent(
          currentRouteMarker.options.studentData,
...
document.getElementById("dataAnalytics").addEventListener("click", () => {
  window.location.href = "/students/data-analytics"
})

document.getElementById("eventReportsLink").addEventListener("click", () => {
